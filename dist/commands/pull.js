...
import { hasLocalChanges, listSkipWorktree, repoRoot, runGitInherit, setSkipWorktree, unmergedPaths, unsetSkipWorktree, } from "../git.js";
/**
...
export function pull(args) {
    ...
    for (const f of saved)
        unsetSkipWorktree(root, f);
    let stashed = false;
    if (hasLocalChanges(root, saved)) {
        const code = runGitInherit(["stash", "push", "--quiet", "-m", "gil-pull", "--", ...saved], root);
        stashed = code === 0;
        if (stashed)
            console.log("gil: stashed your local edits to those files");
    }
    ...
    if (needResolve.length > 0) {
        ...
        for (const f of needResolve)
            console.log(`  ${f}`);
    }
    // Signal that action is needed: non-zero on pull failure or unresolved conflicts.
    if (pullCode !== 0)
        process.exitCode = pullCode;
    else if (needResolve.length > 0)
        process.exitCode = 1;
}
