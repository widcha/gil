...
export function listEntries(file: string): string[] {
  return parse(file).entries;
}

function write(file: string, parsed: Parsed): void {
...
export function addEntry(file: string, entry: string): boolean {
  const parsed = parse(file);
  if (parsed.entries.includes(entry)) return false;
...
export function removeEntry(file: string, entry: string): boolean {
  const parsed = parse(file);
