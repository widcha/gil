...
function gitShows(root: string, rel: string): boolean {
...
function ripgrepShows(root: string, rel: string): boolean | null {
  try {
...
function report(root: string, rel: string, mechanism: string): void {
...
export function status(paths: string[]): void {
  const root = repoRoot();
