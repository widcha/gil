 ...
 */
export function listChangedPaths(root: string): string[] {
  ...
  for (let i = 0; i < status.length; i++) {
    ...
    if (x === "?") continue; // untracked, already collected
    if (existsSync(join(root, path))) tracked.push(path); // excludes deletions
  }
